...
cross_compile_with_native_dune() {
  ...
  mkdir -p _boot
  cp -v "${native_dune}" _boot/dune.exe
  "${native_dune}" build @install -p dune --profile dune-bootstrap
  ...
  cp -rL _build/install/default _build_install_native
  cp _build/default/dune.install _dune.install.saved
  ...
  echo "=== Phase 2: Building native bootstrap tool ==="
  ...
  # The native duneboot runs on build machine but uses cross-compilers in PATH
  ...
  cp -v _boot/dune.exe _build_install_native/bin/dune
  ...
  mv _build_install_native _build/install/default
  mv _dune.install.saved _build/default/dune.install

  # Install using native dune
  echo "=== Installing with native dune ==="
  "${native_dune}" install --prefix="${install_prefix}" dune
...
cross_compile_with_bootstrap() {
  ...
  install_dune_from_build_artifacts "${install_prefix}"
